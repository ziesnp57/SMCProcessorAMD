...
bool SMCProcessorAMD::setupKeysVsmc(){
    ...
    for(int core = 0; core <= this->totalNumberOfPhysicalCores; core++){
        val = MSR_HARDWARE_PSTATE_STATUS_perCore[core];
        VirtualSMCValue *value = VirtualSMCAPI::valueWithSp(0, SmcKeyTypeSp78, val);
        VirtualSMCAPI::addKey(KeyTCxc(core), vsmcPlugin.data, VirtualSMCAPI::valueWithSp(0, SmcKeyTypeSp78, value));
    }
